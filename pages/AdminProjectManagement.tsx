...
const AdminProjectManagement: React.FC = () => {
  ...
  const [newProjectIsPublished, setNewProjectIsPublished] = useState(false);
  ...
  const handleCreateProject = async (event: React.FormEvent) => {
      ...
      .insert([
        {
          ...
          author_id: user.id,
          is_published: newProjectIsPublished,
        },
      ])
      .select();
    ...
    if (error) {
      ...
      setNewProjectIsPublished(false);
      setSelectedImage(null);
  ...
  const handleDeleteProject = async (projectId: string) => {
    ...
    if (projectToDelete?.image_url) {
      const filePath = projectToDelete.image_url.split('blog-images/')[1];
      if (filePath) {
  ...
  if (loading) {
    return <div className="flex items-center justify-center min-h-[calc(100vh-128px)] text-xl font-semibold">Loading project management...</div>;
  }

  if (!user) {
    return <div className="flex items-center justify-center min-h-[calc(100vh-128px)] text-xl font-semibold text-red-500">User not authenticated.</div>;
  }

  return (
    <div className="p-8 bg-white/20 backdrop-blur-md rounded-lg shadow-xl">
      <h3 className="text-2xl font-bold text-slate-800 mb-6">Manage Projects</h3>

      {/* Create New Project Section */}
      <div className="bg-white/30 p-6 rounded-lg shadow-md mb-8">
        <h4 className="text-xl font-semibold mb-4 text-slate-800">Create New Project</h4>
        <form onSubmit={handleCreateProject} className="space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              className="shadow appearance-none border border-gray-300 rounded-lg w-full py-3 px-4 text-slate-700 leading-tight focus:outline-none focus:ring-2 focus:ring-[#F4F754] focus:border-transparent transition-all duration-200 h-24"
          ...
          <div>
            <label htmlFor="projectUrl" className="block text-slate-700 text-sm font-semibold mb-2">Project URL (Live Demo):</label>
            <input
              type="url"
              id="projectUrl"
          ...
          <div>
            <label htmlFor="githubUrl" className="block text-slate-700 text-sm font-semibold mb-2">GitHub URL:</label>
            <input
              type="url"
              id="githubUrl"
          ...
          <div className="flex items-center">
            <input
              type="checkbox"
              id="isPublished" 
              checked={newProjectIsPublished}
              onChange={(e) => setNewProjectIsPublished(e.target.checked)}
              className="mr-2 h-4 w-4 text-[#F4F754] focus:ring-[#F4F754] border-gray-300 rounded"
            />
            <label htmlFor="isPublished" className="text-slate-700 text-sm font-semibold">Publish Project</label>
          </div>
      ...
      <div className="bg-white/30 p-6 rounded-lg shadow-md">
        <h4 className="text-xl font-semibold mb-4 text-slate-800">Your Projects</h4>
        {projects.length === 0 ? (
            ...
            {projects.map((project) => (
              <li key={project.id} className="bg-white/50 p-4 rounded-lg shadow flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div className="flex-grow">
                  <h5 className="text-lg font-bold text-slate-800 mb-1">{project.title}</h5>
                  {project.image_url && <img src={project.image_url} alt={project.title} className="w-24 h-auto my-2 rounded-md shadow-sm" />}
